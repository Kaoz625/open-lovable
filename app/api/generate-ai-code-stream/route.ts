...
const openRouterClients = openRouterKeys.map(key =>
  createOpenAI({
    ...
    headers: {
      'HTTP-Referer': 'https://open-lovable.com',
...
`;
        ...
        // Truncate user prompt to fit within model context limits
        // System prompt is ~16K tokens; reserve space for output too
        const maxInputChars = isGroq ? 28000 : isOpenRouter ? 40000 : isGoogle ? 120000 : 150000;
        let truncatedPrompt = fullPrompt;
        if (fullPrompt.length > maxInputChars) {
          truncatedPrompt = fullPrompt.substring(0, maxInputChars) +
            '\n\n[Content truncated to fit model context limit — focus on the visible sections above]';
          console.log(`[generate-ai-code-stream] Prompt truncated from ${fullPrompt.length} to ${maxInputChars} chars for ${providerName}`);
        }

        // Per-model output token limit
        const maxOutputTokens = (isGroq || isOpenRouter) ? 4096 : 8192;
...
It's better to have 3 complete files than 10 incomplete files.`
        ...
        while (retryCount <= maxRetries) {
          try {
            ...
            const isRetryableError = streamError.statusCode === 429 ||
                                    streamError.statusCode === 503 ||
                                    streamError.message?.includes('Service unavailable') ||
                                    streamError.message?.includes('rate limit') ||
                                    streamError.message?.includes('Rate limit') ||
                                    streamError.message?.includes('rate-limited') ||
                                    streamError.message?.includes('Provider returned error') ||
            ...
            if (retryCount < maxRetries && isRetryableError) {
              ...
              // On final retry for non-Anthropic providers: fall back to Claude Haiku
              if (retryCount === maxRetries && !isAnthropic) {
                console.log(`[generate-ai-code-stream] ${providerName} failed all retries, falling back to Claude Haiku`);
                await sendProgress({
                  type: 'info',
                  message: `${providerName} unavailable — falling back to Claude Haiku...`
                });
                streamOptions.model = anthropic('claude-haiku-4-5-20251001');
                actualModel = 'claude-haiku-4-5-20251001';
              } else if (isOpenRouter && openRouterClients.length > 1) {
                // Rotate to next OpenRouter key
        ...
        // Use fullStream (not textStream) so API errors are not silently dropped
        for await (const part of result?.fullStream || []) {
          if (part.type === 'error') {
            console.error('[generate-ai-code-stream] Stream error event:', part.error);
...
Provide the complete file content without any truncation. Include all necessary imports, complete all functions, and close all tags properly.`;
                ...
                // Determine the correct model name for the completion (same as actualModel)
                let completionModelName: string = actualModel;
