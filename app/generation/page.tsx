...
import { PropertiesPanel, type ElementProps } from '@/components/PropertiesPanel';
...
function AISandboxPage() {
  ...
  // Visual inspector: listen for postMessage events from the sandboxed iframe
  useEffect(() => {
    const handleMessage = (e: MessageEvent) => {
      if (!e.data?.type?.startsWith('inspector:')) return;
      if (e.data.type === 'inspector:select') {
        setSelectedElement(e.data as ElementProps);
      }
      if (e.data.type === 'inspector:moved') {
        addChatMessage(e.data.hint || 'Element moved in preview — click Apply to Code to save.', 'system');
      }
      if (e.data.type === 'inspector:text-changed') {
        addChatMessage(`Text edited in preview: "${e.data.text?.substring(0, 80)}" — click Apply to Code to save.`, 'system');
      }
    };
      ...
      if (sandboxData?.url) {
        return (
          <div className="relative w-full h-full">
            ...
            />
            {/* Properties Panel — shown when inspector is active and element is selected */}
            {inspectorActive && selectedElement && (
              <PropertiesPanel
                element={selectedElement}
                iframeRef={iframeRef}
                onApplyToCode={(prompt) => {
                  setAiChatInput(prompt);
                  // Focus will land on chat input
                }}
                onClose={() => {
                  setSelectedElement(null);
                  iframeRef.current?.contentWindow?.postMessage({ type: 'inspector:disable' }, '*');
            ...
            <button
              onClick={async () => {
                if (!inspectorActive) {
                  // Inject the inspector script into the sandbox
                  try {
                    await fetch('/api/inject-inspector', { method: 'POST' });
                    // Reload iframe so index.html picks up the new script tag
                    if (iframeRef.current && sandboxData?.url) {
                      iframeRef.current.src = `${sandboxData.url}?t=${Date.now()}&inspector=1`;
                    }
                    // Wait for iframe to reload then send enable message
                    setTimeout(() => {
                      iframeRef.current?.contentWindow?.postMessage({ type: 'inspector:enable' }, '*');
                    }, 2500);
                    setInspectorActive(true);
                    setSelectedElement(null);
                  } catch (e) {
                    console.error('[Inspector] Failed to inject:', e);
                  }
                } else {
                  // Disable inspector
                  iframeRef.current?.contentWindow?.postMessage({ type: 'inspector:disable' }, '*');
              ...
              className={`absolute bottom-4 right-14 p-2 rounded-lg shadow-lg transition-all duration-200 hover:scale-105 ${
                inspectorActive
                  ? 'bg-indigo-500 text-white hover:bg-indigo-600'
                  : 'bg-white/90 hover:bg-white text-gray-700'
              }`}
              title={inspectorActive ? 'Disable visual selector (click elements to edit)' : 'Enable visual selector (click elements to edit)'}
            >
              <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
Focus on the key sections and content, making it clean and modern.`;
        ...
        // Abort any existing generation and create a new controller
        ...
        // Silently stop on AbortError (model switch or intentional cancel)
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (generationProgress.isGenerating && generationAbortRef.current) {
                ...
                setGenerationProgress(prev => ({ ...prev, isGenerating: false, isStreaming: false, status: '' }));
                addChatMessage(`Switched to ${newModel} — start a new request to use it.`, 'system');
              }
            }}
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 max-w-[400px] flex flex-col border-r border-border bg-background">
          ...
          <div className="p-4 border-t border-border bg-background-base">
            ...
            {selectedElement && (
              <div className="flex items-center gap-2 mb-2 px-3 py-1.5 bg-indigo-50 border border-indigo-200 rounded-lg text-xs">
                <span className="w-2 h-2 bg-red-400 rounded-full flex-shrink-0" />
                <span className="text-indigo-700 font-mono truncate flex-1">
                  {selectedElement.hint
                    ? `${selectedElement.hint} <${selectedElement.tag}>`
                    : `<${selectedElement.tag}>${selectedElement.classes ? ' .' + selectedElement.classes.split(' ')[0] : ''}`}
                </span>
                <button
                  onClick={() => { setSelectedElement(null); setAiChatInput(''); }}
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1 text-xs text-gray-500 hover:text-gray-800 border border-gray-200 hover:border-gray-400 rounded-md transition-colors"
                title="Import HTML file to recreate as React"
              >
            ...
            <HeroInput
              ...
              placeholder={inspectorActive ? 'Click any element in the preview to select it…' : 'Describe what you want to build...'}
